        ...
        $(function() {
			...
			$.getJSON("plist", {beuid:$("#beuid").val()}, function(r){
            	...
            	$("#btnsub").click(function(){
						var beuid = $("#beuid").val();
						var sbeuid = $("#sbeuid").val();
						var data = "beuid="+beuid+"&sbeuid="+sbeuid;
						$.ajax({url:"insertsub", data:data, success:function(r){
								if(r == "1"){
									alert("구독 완료!");
									$("#btnsub").html("구독중").attr("id","btnsubok").css("background-color","gray");
								}
								else{
									alert("구독 실패");
								}
...
<body>
   <input type="hidden" name="sbeuid" id="sbeuid" value="${sessionScope.beuid }"> 
    <div class = "container myinfo">
        <div class="row-fluid">
            <div class="span2" >
                <img src="img/${ch.ch_img }" style="width:140px; height: 140px;" class="img-circle imgC" id="imgC">
            </div>
            <input type="hidden" id="beuid" value="${user.beuid }">
           <input type="hidden" id="msg" value="${msg}">
           <div class="span8">
                <h2><i class="oldname">${ch.ch_name}</i> </h2>
                <h4><i class="oldname">${user.uname}님의 채널</i> </h4>
                <h6><i>Email: </i><i name="email" class="cemail">${user.email}</i></h6>
                ...
                <div class="btn-group">
                     ...
                     <c:choose>
                     	<c:when test="${sub eq 'yes'}">
		                    <a class="btn dropdown-toggle btn-info" id="btnsubok" data-toggle="dropdown" style="background-color:gray;">
		                       구독중
		                    </a>
		                    </c:when>
		                    <c:otherwise>
		                    	<a class="btn dropdown-toggle btn-info" id="btnsub" data-toggle="dropdown">
		                       구독하기
		                    </a>
		                    </c:otherwise>
                    </c:choose>
